...
(function (CountlyHelpers, $, undefined) {
	...
	CountlyHelpers.randomId = function (len) {
	...
	CountlyHelpers.buildMenu = function (items) {
		var seperator = false;
		var t = $( '<select />', {'id':items.selectId} );

		var s = (items.selected != undefined) ? false : true;
		if(items.showheader === true) {
	    	$('<option />', { selected: s, value: '', text: '[-nothing selected-]' }).appendTo(t);
		}
		for(var idx in items.datapack) {
			var sel = (items.datapack[idx][items.value] == items.selected) ? true : false;
			
		    $('<option />', { selected: sel, value: items.datapack[idx][items.value], text: items.datapack[idx][items.text] }).appendTo(t);
		}
		t = $('<div class="inner" style="text-align:center;" />').wrapInner(t);
		t = $('<div class="item help-zone-vs" />').wrapInner(t);
		t = $('<div id="pulldown-container" class="dashboard-summary block_'+items.selectId+' inarow' + ((seperator === true)?' seperatedpd" />':'" />')).wrapInner(t);
		return($(t));
	};

	CountlyHelpers.doMenu = function (arr, v, n, i, s, h, f) {
		$('#pbarea').append(
			CountlyHelpers.buildMenu( {	'datapack': 	arr, 
										'selected': 	s,
										'value': 		v, 
										'text': 		n, 
										'showheader': 	h,
		...
		$(document).off('change', '#'+i);
		if(typeof f != 'undefined'){
			$(document).on('change', '#'+i, f);
		}
	};

	CountlyHelpers.doButton = function (sel, f) {
		var t = $('<button />', sel);
		t = $('<div class="inner" style="text-align:center;" />').wrapInner(t);
		t = $('<div class="item help-zone-vs" />').wrapInner(t);
		t = $('<div id="pulldown-container" class="dashboard-summary inarow block_'+sel.id+' " />').wrapInner(t);
		$('#pbarea').append($(t));
		if(typeof sel != 'undefined' && typeof f != 'undefined') {
			$(document).on( "click", "#"+sel.id, f );
		}
	};
...
/*
	TODO BUG: sequential get calls result with multiple dropdowns for campaigns
*/
/*
	TODO do i really need to call getdashboardreportmap twice???
*/
/*
	TODO after value entered, the next time when editing occurs, it retrieves the first (unchanged) value for the input field
*/
...
window.SDKView = countlyView.extend({
    initialize:function () {
        this.template = Handlebars.compile($("#sdkwizard-template").html());
    },
...
window.ManagementView = countlyView.extend({
    initialize:function () {
        this.template = Handlebars.compile($("#sdkwizard-template").html());
    },
    render:function (eventName) {
...
window.MakinaView = countlyView.extend({
	views: {},
	pulldowns: {},
    initialize:function () {
		this.statics();
        this.template = Handlebars.compile($("#makina-template").html());
    },
	statics:function () {
		this.views = {
				'mcommerce/overview': {
					...
					// val: template id (from createPulldown()) of the select menu
					'upper': {'graph1': 'pdA', 'graph2': 'pdA'},
					// buttons object:
					// key: id to be generated
					// val: button title
					'buttons': {'ts': 'Total Sales', 'tr': 'Total Revenue', 'mccr': 'M-Commerce Cr'},
					// buttons object:
					// key: id to be generated
					// val: template id (from createPulldown()) of the select menu
					'secondary': {'secondary': 'pdD'},
					// table object:
					// key: id to be generated
					// val: column title
					'table': [
				...
				'qrcodeman/qr-drilldown/overview': {
					'upper': {},
					'buttons': {'ts': 'Total Scans', 'ti': 'Total Installs', 'cr': 'Conversion Rate'},
					'secondary': {'secondary': 'pdE'},
					'table': [
						{'title': 'Date'},
						{'title': 'Click'},
						{'title': 'Install'},
				...
				'qrcodeman/my-qr-campaigns/overview': {
					'verticalList': true,
					'upper': {},
					'buttons': {},
					'secondary': {'secondary': 'pdD'},
				...
				'sources/source-drilldown/mcommerce': {
					'verticalList': true,
					'upper': {
						'graph1': 'pdA', 
						'graph2': 'pdA'
				...
				'sources/source-drilldown/events': {
					'verticalList': true,
					'upper': {'graph1': 'pdB', 'graph2': 'pdB'},
					'buttons': {'goal1': 'Top Goal Conversions', 'goal2': '2nd Goal Conversions', 'ocr': 'Overall Conversion Rate'},
					'secondary': {'secondary': 'pdF'},
					'table': [
						{'title': 'Campaign'},
						{'title': 'Goal Name'},
						{'title': 'Total Count'},
				...
				'sources/source-drilldown/users-by-source': {
					'tabbedGraph': true,
					'verticalList': true,
					'upper': {},
					'buttons': {'tu': 'Total Users', 'nu': 'New Users', 'ru': 'Returning Users'},
					'secondary': {'secondary': 'pdG'},
					'table': [
						{'title': 'Campaign'},
						{'title': 'Total Users'},
						{'title': 'New Users'},
		...
		/*
				'sources/source-drilldown': {
					'verticalList': true,
					'upper': {'graph1': 'pdC', 'graph2': 'pdC'},
					'buttons': {'tu': 'Total Users', 'nu': 'New Users', 'ru': 'Returning Users'},
					'secondary': {'secondary': 'pdG'},
					'table': {
						'columns': {
		...
		*/
				'sources/overview': {
					'upper': {'graph1': 'pdC', 'graph2': 'pdC'},
					'buttons': {'tu': 'Total Clicks', 'ti': 'Total Installs', 'avgcpd': 'Avg CPD', 'total': 'Total Spent'},
					'secondary': {'secondary': 'pdH'},
					'table': [
						{'title': 'Channel'},
						{'title': 'Clicks'},
						{'title': 'Installs'},
			...
			this.pulldowns = {
				'pdA': {'sales': 'Total Sales', 'convrate': 'Conversion Rate', 'revenue': 'Total Revenue'},
				'pdB': {'conversions': 'Total Conversions', 'convrate': 'Conversion Rate', 'signup': 'Signup', 'watch': 'Watch'},
				'pdC': {'clicks': 'Clicks', 'installs': 'Installs', 'cost': 'Cost', 'cpd': 'CPD', 'cpc': 'CPC'}, 
				'pdD': {'source': 'Source', 'campaign': 'Campaign', 'date': 'Date', 'country': 'Country', 'appv': 'App Version', 'device': 'Device'}, 
				'pdE': {'appv': 'App Version', 'platform': 'Platform', 'device': 'Device'}, 
				'pdF': {'date': 'Date', 'appv': 'App Version', 'platform': 'Platform', 'device': 'Device', 'maklink': 'Maklink Parameter'}, 
				'pdG': {'date': 'Date', 'appv': 'App Version', 'maklink': 'Maklink Parameter'}, 
				'pdH': {'campaign': 'Campaign', 'date': 'Date'}, 
			};
	},
    render:function (eventName) {
        this.renderCommon();
...
//			countlyHandsome.refreshTable(app);
			this.buildViews();
            this.pageScript();
	...
	createPulldown:function (dx, sel, seperator) {
		sel = typeof sel !== 'undefined' ? sel : {};

		var t = $('<select />', sel);
		for(var val in this.pulldowns[dx]) {
		    $('<option />', {value: val, text: this.pulldowns[dx][val]}).appendTo(t);
		}
		t = $('<div class="inner" style="text-align:center;" />').wrapInner(t);
		t = $('<div class="item help-zone-vs" />').wrapInner(t);
		t = $('<div id="pulldown-container" class="dashboard-summary inarow' + ((seperator === true)?' seperatedpd" />':'" />')).wrapInner(t);
		return($(t));
	},
	createButton:function (sel) {
		...
		t = $('<div class="inner" style="text-align:center;" />').wrapInner(t);
		t = $('<div class="item help-zone-vs" />').wrapInner(t);
		t = $('<div id="pulldown-container" class="dashboard-summary inarow" />').wrapInner(t);
		return($(t));
	},
	buildViews:function (){
		// NOTE:
		// If there are two paths overlapping in the views opject, it renders the view elements twice.
		// e.g. makina/a/b/c and makina/a/b
		$('#pdarea, #pbarea').html('');
		var url = window.location.href;
		if(url.indexOf('/makina/') > 0) {
			for (var vkey in this.views) {
				if(url.indexOf(vkey) > 0) {
					var set = this.views[vkey];
					if(Object.keys(set.upper).length > 0) {
						for (var ukey in set.upper) {
//							this.createPulldown(set.upper[ukey], {id: ukey, class: 'pulldownClass'}, false).appendTo('#pdarea');
						}
						$('#pdarea').append('<br class="clx" />');
					}

					if(Object.keys(set.buttons).length > 0) {
						for (var bkey in set.buttons)
							this.createButton({id: bkey, text: set.buttons[bkey]}).appendTo('#pbarea');
					}
					if(Object.keys(set.secondary).length > 0) {
						for (var skey in set.secondary) {
//							this.createPulldown(set.secondary[skey],  {id: skey, class: 'pulldownClass'}, true).appendTo('#pbarea');
						}
						$('#pbarea').append('<br class="clx" />');
					}
				}
			}
		}
		$('#pcarea').append( countlyHandsome.getTablePart('total') );
		chainedPulldowns();
	},
});
...
var AppRouter = Backbone.Router.extend({
    routes:{
		"/": 												"dashboard",
		"/makina": 											"makina",
		"/makina/mcommerce/overview": 						"makina",
		"/makina/qrcodeman/qr-drilldown/overview": 			"makina",
		"/makina/qrcodeman/my-qr-campaigns/overview": 		"makina",
		"/makina/sources/source-drilldown/mcommerce": 		"makina",
		"/makina/sources/source-drilldown/events": 			"makina",
		"/makina/sources/source-drilldown/users-by-source": "makina",
		"/makina/sources/overview": 						"makina",

//		"/makina/sources/overview/:app/:camp/:channel/:makilink/?:params": 'makina',

		"/makina/sources/overview/app/:appId": 													"makinaWithApp",
		"/makina/sources/overview/app/:appId/cmp/:compId": 										"makinaWithAppComp",
		"/makina/sources/overview/app/:appId/cmp/:compId/ch/:channelId": 						"makinaWithAppCompChannel",
		"/makina/sources/overview/app/:appId/cmp/:compId/ch/:channelId/maki/:makilinkId": 		"makinaWithAppCompChannelMakilink",
	
		"/makina/sources/source-drilldown": 				"makina",
		"/users/sessions": 									"sessions",
		"/users/countries": 								"countries",
		"/users/users": 									"users",
		"/users/loyalty": 									"loyalty",
		"/users/devices": 									"devices",
		"/users/platforms": 								"platforms",
		"/users/versions": 									"versions",
		"/users/carriers": 									"carriers",
		"/users/frequency": 								"frequency",
//		"/users/events": 									"uevents",
        "/events": 											"events",
    ...
    makinaWithAppCompChannelMakilink:function (appId, compId, channelId, makilinkId) {
        this.renderWhenReady(this.MakinaView);
    },
    makinaWithAppCompChannel:function (appId, compId, channelId) {
        this.renderWhenReady(this.MakinaView);
    },
    makinaWithAppComp:function (appId, compId) {
        this.renderWhenReady(this.MakinaView);
    },
    makinaWithApp:function (appId) {
        this.renderWhenReady(this.MakinaView);
    },
	makina:function () {
        this.renderWhenReady(this.MakinaView);
	...
	management:function () {
        this.renderWhenReady(this.ManagementView);
    },
    sdkios:function () {
        this.renderWhenReady(this.SDKView);
    },
    sdkandroid:function () {
        this.renderWhenReady(this.SDKView);
    },
    ...
    initialize:function () { //initialize the dashboard, register helpers etc.
        ...
        this.MakinaView = new MakinaView();
...
chainedPulldowns = function () {
	if(countlyHandsome.getappId() == false) {
		countlyHandsome.setappId( countlyHandsome.getHAIDwithCAID( countlyGlobal['apps'][countlyCommon.ACTIVE_APP_ID]._id ) );
	}
	countlyHandsome.initialCalls();
	
	$('#pbarea, #pcarea').html('');
	var doAllPulldownsRenderTheTable = false;

	CountlyHelpers.doMenu( [{'l':0, 'f':'App'}, {'l':1, 'f':'Channel'}, {'l':2, 'f':'Campaign'}, {'l':3, 'f':'Makilink'}], 'l', 'f', 'selectFraction', countlyHandsome.getfractionLevel(), false, function () {
		countlyHandsome.setfractionLevel( $(this).val() );

		if(doAllPulldownsRenderTheTable) {
			countlyHandsome.refreshTable( {
											'appId': countlyHandsome.getappId(), 
											'channelId': countlyHandsome.getchannelId(),
											'fractionLevel': countlyHandsome.getfractionLevel(),
											'campId': '', 'makilinkId': '', 'reportTypeKeyName': ""
			} );
		}
	});

	if(countlyHandsome.appsList != 'undefined' && countlyHandsome.getappId() != "") {
		CountlyHelpers.doMenu( countlyHandsome.appsList, 'appId', 'appname', 'selectApp', countlyHandsome.getappId(), false, function () {
			countlyHandsome.setappId( $(this).val() );
		});
	}
	if(countlyHandsome.channelsList != 'undefined' && countlyHandsome.getappId() != "") {
		CountlyHelpers.doMenu( countlyHandsome.channelsList, 'channelId', 'channelKeyName', 'selectChannel', '', true, function () {
			countlyHandsome.setchannelId( $(this).val() );
		});
	}
	if(countlyHandsome.getappId() != "") {
		countlyHandsome.refreshTable( {	'reportTypeKeyName': "", 'fractionLevel': countlyHandsome.getfractionLevel(),
										'appId': countlyHandsome.getappId(), 'channelId': countlyHandsome.getchannelId(),
										'campId': countlyHandsome.getcampId(), 'makilinkId': countlyHandsome.getmakilinkId() }, 0);
	}

	//
	// List the campaigns, use the Application and Channel menus as parameters
	//
	
	CountlyHelpers.doButton({id: 'GetCampaignsButton', text: 'Get'}, function () {

//		countlyHandsome.showCache();
		countlyHandsome.setcampId('');
		countlyHandsome.setmakilinkId('');
		$('#pulldown-container.block_selectCampaign').remove();
		$('#pulldown-container.block_selectMakilink').remove();
		$.event.trigger({'type': "cleanYourself", 'message': '', 'time': new Date()});
		countlyHandsome.refreshTable( {	'reportTypeKeyName': "", 'fractionLevel': countlyHandsome.getfractionLevel(),
										'appId': countlyHandsome.getappId(), 'channelId': countlyHandsome.getchannelId(),
										'campId': countlyHandsome.getcampId(), 'makilinkId': countlyHandsome.getmakilinkId() }, 1);
	});

	$(document).on( "ListCampaignReceivedAndProccessedData", function () {
		$('.block_selectCampaign').remove();
		CountlyHelpers.doMenu( countlyHandsome.campaignsList, 'campId', 'name', 'selectCampaign', '', true, function() {
			countlyHandsome.setcampId( $(this).val() );
			countlyHandsome.setmakilinkId('');
/*
		});
		CountlyHelpers.doButton({id: 'GetMakilinksButton', text: 'Get'}, function () {
			$(this).parents('div#pulldown-container').remove();
*/
			$.event.trigger({'type': "cleanYourself", 'message': '', 'time': new Date()});
			countlyHandsome.refreshTable( {	'reportTypeKeyName': "", 'fractionLevel': countlyHandsome.getfractionLevel(),
											'appId': countlyHandsome.getappId(), 'channelId': countlyHandsome.getchannelId(),
											'campId': countlyHandsome.getcampId(), 'makilinkId': countlyHandsome.getmakilinkId() }, 2);
		});
	} );
	$(document).on( "ListMakilinksReceivedAndProccessedData", function () {
		$('.block_selectMakilink').remove();
		CountlyHelpers.doMenu( countlyHandsome.makilinksList, 'makilinkId', 'makiHumanKey', 'selectMakilink', '', true, function() {
			countlyHandsome.setmakilinkId( $(this).val() );
/*
		});
		CountlyHelpers.doButton({id: 'GetTableButton', text: 'Get'}, function () {
*/
			$.event.trigger({'type': "cleanYourself", 'message': '', 'time': new Date()});
			countlyHandsome.refreshTable( {	'reportTypeKeyName': "", 'fractionLevel': countlyHandsome.getfractionLevel(),
											'appId': countlyHandsome.getappId(), 'channelId': countlyHandsome.getchannelId(),
