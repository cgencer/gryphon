...
// Private Methods
		function init() {
			this.cleanUp();
			cuteNSexy.getSessionID();
		};
		function buildUrl (cmd, data) {
...
//			cuteNSexy.Logger.dir(payLoad);
			...
			var newCall = CryptoJS.SHA256(cuteNSexy.buildUrl(payLoad.command, payLoad.data)).toString(CryptoJS.enc.Hex);
			alreadyCalledBefore = (newCall === cuteNSexy._lastCall) ? true : false;
...
// Private Methods
		...
		baseObj: function(sid) { return ({'callTag': "", 'registerId': "", 'verb': "", 'session': sid, 'callbackTag': this.buffer.salt });},
