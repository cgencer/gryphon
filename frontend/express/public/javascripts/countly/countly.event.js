(function(countlyEvent, $, undefined) {
	...
	countlyEvent.getMultiEventData = function(eventKeysArr, packet, callback) {
	    _activeAppId = countlyCommon.ACTIVE_APP_ID;
		var _newId = (eventKeysArr instanceof Array) ? eventKeysArr.join().replace(/[0@_.,]/g,'') : eventKeysArr;
		var hashed = CryptoJS.SHA256(_newId).toString(CryptoJS.enc.Hex);
		if(typeof _reqBuffer == 'undefined') {
			var _reqBuffer = new Array();
		}
		_reqBuffer[hashed] = eventKeysArr;
	    ...
	    $.ajax({
	        ...
	        dataType: "jsonp",
	        success: function(json) {
				callback( extractTotalCount(json, hashed), packet );
	        }
    	});

	    function extractTotalCount(dataFromDb, eventLabel) {
			if(typeof eventLabel === 'undefined') {
//				console.info('empty eventLabel found!');
//				console.info('labeled as : '+_reqBuffer[eventLabel].join(','));
			}else if(_.isEmpty( dataFromDb )) {
//				console.info('eventLabel resulting empty JSON recordset found!');
//				console.info('labeled as : '+_reqBuffer[eventLabel].join(','));
				countlyHandsome.savePlot(eventLabel, {'2013':{'1':{'1':{'1':{'c':0},'c':0},'c':0},'c':0}}, 'year');
			}else{
				countlyHandsome.savePlot(eventLabel, dataFromDb, 'year');
			}

			var total = 0;
			if(_.isEmpty( dataFromDb )) {
				total = 0;
			}else{
				for(var idx in dataFromDb){
					total += Number(dataFromDb[idx].c);
				}
			}
			return({ 'value': total, 'label': eventLabel});
		}
	};
	
	countlyEvent.extractDataForGraphAndChart = function (dataFromDb) {
        var eventData = {};
		var eventMap = (_activeEvents) ? ((_activeEvents.map) ? _activeEvents.map : {}) : {};
		var countString = (eventMap[_activeEvent] && eventMap[_activeEvent].count) ? eventMap[_activeEvent].count : jQuery.i18n.map["events.table.count"];
		var sumString = (eventMap[_activeEvent] && eventMap[_activeEvent].sum) ? eventMap[_activeEvent].sum : jQuery.i18n.map["events.table.sum"];
